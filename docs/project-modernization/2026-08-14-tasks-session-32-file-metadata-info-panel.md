...
- **Size / Created / Modified / Owner** — from disk on desktop; on the web
  ...
  available.
- **Generated by** — the tool/agent that produced the document, when it can be
  inferred. The OS never records the creating *application* (owner is only the OS
  user), so this is detected from the **document content** — the one place a
  generator can leave a mark. Cross-surface (parses `rawMarkdown`, so it works on
  web too). Detection order, highest confidence first: (1) YAML front-matter keys
  (`generator`/`creator`/`created_by`/`tool`/`author`/…); (2) an HTML comment
  marker (`<!-- specdown:creator=… -->` or a generic `<!-- Generated by … -->`);
  (3) a narrow set of footer signatures (e.g. "Generated with Claude Code",
  `Co-Authored-By: Claude`, "made with Cursor"). Signature matching is
  deliberately tight so a bare mention of a tool in prose is **not** mistaken for
  authorship, and the row only appears when a signal is actually found. Lives in
  `features/creator-detect.js` (`detectCreator`), covered by
  `tests/unit/creatorDetect.test.js`.
