...
describe('File info sheet (web)', () => {
  ...
  describe('pure helpers', () => {
    ...
    it('adds a "Generated by" row when the content signals a creator', () => {
      const tab = {
        filename: 'gen.md',
        filePath: null,
        rawMarkdown: '---\ngenerator: Cursor\n---\n# Doc',
      ...
      const rows = buildInfoRows(tab);
      const gen = rows.find((r) => r.key === 'generator');
      expect(gen).toBeTruthy();
      expect(gen.label).toBe('Generated by');
      expect(gen.value).toBe('Cursor');
      expect(gen.title).toMatch(/front-matter/i);
    });

    it('omits the "Generated by" row when there is no signal', () => {
      const tab = { filename: 'plain.md', filePath: null, rawMarkdown: '# Plain', sourceMeta: {} };
      expect(buildInfoRows(tab).some((r) => r.key === 'generator')).toBe(false);
    });
  });
